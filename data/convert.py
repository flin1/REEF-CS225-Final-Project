...
def convert_airports_csv():
    ...
    # df_airports = df_airports.replace('','-', regex=True)
    ...
    df_airports.to_csv('airports.csv', index = False, header = False)
def convert_routes_csv():
    ...
    df_routes.to_csv('routes.csv', index = False, header = False)
