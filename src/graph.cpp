...
vector<int> Graph::Dijkstra(int origin_airport_id, int destination_airport_id) {
    ...
    auto comparator = [](pair<double, AirportNode> input1, pair<double, AirportNode> input2) { return input1.first > input2.first; };
    priority_queue<pair<double,AirportNode>, vector<pair<double,AirportNode> >, decltype(comparator) > priorityQueue(comparator);
    priorityQueue.push({0,start});

    while (priorityQueue.top().second.id != destination_airport_id) {
        AirportNode current_airport = priorityQueue.top().second;
        priorityQueue.pop();
        for (AirportNode neighbor : current_airport.) {
